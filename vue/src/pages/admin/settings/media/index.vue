<template>
  <div class="max-w-4xl mt-12 md:mt-24 mx-auto grid items-center justify-center grid-cols-1 md:grid-cols-2 gap-8">
    <Toast />
    <div class="flex flex-col space-y-8">
      ...
      <div class="flex-input m2">
        ...
        <input
          ...
          class="bg-white/30 focus:outline-none focus:ring-0 w-full text-center"
          type="text"
    ...
    <div class="flex flex-col">
      <label class="text-lg text-gray-500" for="phone_number">Phone Number</label>
      <input
        type="tel"
        name="phone_number"
        id="phone_number"
        class="bg-gray-200 rounded-lg shadow-sm px-2 py-3 my-2 focus:ring-green-700 focus:outline-green-700"
        v-model="settings.phone_number"
      />

      <label class="text-lg text-gray-500" for="email">Email</label>
      <input
        type="email"
        name="email"
        id="email"
      ...
      <label class="text-lg text-gray-500" for="currency">Default Currency</label>
      <input
        type="text"
        name="currency"
        id="currency"
  ...
  <div class="text-center mx-auto">
    <button class="py-3 px-8 my-8 bg-green-700 rounded-lg text-white font-bold text-lg" @click="updateSettings">
      Update
    </button>
  </div>
</template>
<script setup>
  ...
  const getSettings = () => {
    axios
      ...
      .then((res) => {
        ...
        settings.value.id = res.data.setting.id
        settings.value.phone_number = res.data.setting.phone_number
        settings.value.email = res.data.setting.email
        settings.value.currency = res.data.setting.currency
        settings.value.social_links = JSON.parse(res.data.setting.social_links).links
        toast.add({ severity: 'success', summary: 'Successful', detail: 'settings updated successfully', life: 3000 })
      })
      .catch((err) => {
        console.log(err)
      })
  ...
  const updateSettings = () => {
    ...
    formData.append('social_links', JSON.stringify({ links: settings.value.social_links }))
    axios
      .post(`api/settings/${settings.value.id}`, formData)
      .then((res) => {
        console.log(res)
        toast.add({ severity: 'success', summary: 'Successful', detail: 'settings updated successfully', life: 3000 })
      })
