...
const fetchData = () => {
  ...
  axios.get("/api/flights").then((res) => {
    loading.value = false
    elements.value = res.data.flights
...
const openNew = () => {
router.push({ name: 'flight-create' })
}
...
const edit = (edit) => {
  user.value = {...edit}
  router.push({ name: 'flight-update',params:{id:user.value.id} })
}
...
const deleteAction = () => {
  axios
    .delete(`/api/flights/${element.value.id}/delete`)
    .then((res) => {
      console.log(res.data)
        ...
        >
          <template #header>
            <div class="flex flex-column md:flex-row md:justify-between md:align-items-center">
              <h5 class="m-0">Manage Flight Reservations</h5>
          ...
          <Column field="link" header="link" :sortable="true" header-style="width:14%; min-width:10rem;">
            <template #body="slotProps">
              {{ slotProps.data.link }}
            </template>
          </Column>
          <Column field="discount" header=" discount" :sortable="true" header-style="width:14%; min-width:10rem;">
            <template #body="slotProps">
