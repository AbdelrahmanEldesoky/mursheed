...
const fetchData = () => {
  ...
  axios.get("/api/attracives").then((res) => {
    loading.value = false
    elements.value = res.data.attractives
...
const openNew = () => {
router.push({ name: 'Attract-create' })
}
...
const edit = (edit) => {
  user.value = {...edit}
  router.push({name:'Attract-update',params:{'id':user.value.id} })
}
...
const deleteAction = () => {
  loading.value=true;
  axios
    .delete(`/api/attracives/${user.value.id}/delete`)
    .then((res) => {
      ...
      fetchData()
      loading.value=false
      toast.add({severity: 'success', summary: 'Successful', detail: 'attracives deleted', life: 3000})
    })
    .catch(() => {
    })
        ...
        >
          <template #header>
            <div class="flex flex-column md:flex-row md:justify-between md:align-items-center">
              <h5 class="m-0">Manage attracives</h5>
          ...
          <Column field="country_name" header="country_name" :sortable="true" header-style="width:14%; min-width:10rem;">
            <template #body="slotProps">
              {{ slotProps.data.country.country }}
            </template>
          </Column>
          <Column field="city_name" header="city_name" :sortable="true" header-style="width:14%; min-width:10rem;">
            <template #body="slotProps">
              {{ slotProps.data.state.state }}
            </template>
          </Column>
          <Column field="lat" header="lat" :sortable="true" header-style="width:14%; min-width:10rem;">
            <template #body="slotProps">
              {{ slotProps.data.lat }}
            </template>
          </Column>
          <Column field="long" header="long" :sortable="true" header-style="width:14%; min-width:10rem;">
            <template #body="slotProps">
              {{ slotProps.data.long }}
            </template>
          </Column>
